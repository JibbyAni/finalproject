 ...
 */
var connection_string = 'localhost:27017/brigades';
/*
 ...
 * retrieve, and update functions just return the data directly?
 ...
 */
 ...
 exports.delete = function(collection, query, callback){
    mongoDB.collection(collection).remove(query, 
      ...
      function(err, status) {   // callback upon completion
      if (err) doError(err);
      console.log("11. Done with mongo delete operation in mongoModel");
      // use the callback function supplied by the controller to pass
      // back true if successful else false
      var success = (status.result.n == 1 ? true : false);
      callback(success);
      console.log("12. Done with delete operation callback in mongoModel");
    });

   console.log("13. Done with delete function in mongoModel");
 }
